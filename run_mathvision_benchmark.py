"""
run_mathvision_benchmark.py

Speculative decoding benchmark on specific MathVision questions.
Adapted from run_h200_benchmark.py but uses mathvision_vl.json
and matches QIDs: 5, 133, 123, 168, 117, 104.
"""
...
SAMPLING_KWARGS = dict(
    ...
    max_tokens=40960,  # Updated to match SpecReason setup
    seed=42,
    extra_body={
        "top_k": 20,
...
def run_benchmark(server_base: str, mode: str, dataset_path: str) -> list[dict]:
    ...
    prompts = [item for item in full_dataset if item.get("qid") in TARGET_QIDS]
    # sort them by QID to be consistent
    prompts = sorted(prompts, key=lambda x: x["qid"])
    ...
    print(f"Prompts: {len(prompts)} mathvision questions")
    print(f"{'='*70}")
    ...
    for prompt in prompts:
        ...
        has_img  = bool(prompt.get("image_b64"))
        ...
        print(
            f"  QID {pid:<18} {img_tag}  lat={latency:6.2f}s  "
...
def print_comparison(spec_results: list[dict], base_results: list[dict]) -> None:
    ...
    for pid in all_ids:
        ...
        print(f"  QID {pid:<20}  {b_tps:>14.1f}  {s_tps:>10.1f}  "
...
def parse_args() -> argparse.Namespace:
    p   = argparse.ArgumentParser(description="H200 spec-decode benchmark on MathVision")
    ...
    run_p.add_argument("--dataset",  default="mathvision_vl.json", help="Path to mathvision dataset")
...
def main() -> None:
    ...
    # --- run ---
    results = run_benchmark(args.server, args.mode, args.dataset)
