...
echo "Node layout:"
echo "  Node1 (32B scorer):  $NODE1  →  $VLLM_32B_URL"
echo "  Node2 (2B drafter):  $NODE2  →  $VLLM_2B_URL"
...
start_server_on_node() {
    ...
    echo "=== Starting $label on $node:$port ==="
...
if [ -f "$RESULTS_DIR/run_specReason_results.json" ]; then
    echo "=== SpecReason — already done, skipping ==="
else
    echo "=== SpecReason (32B scorer + 2B drafter, 2 nodes) ==="

    # Start 32B scorer on node 1
    start_server_on_node "$NODE1" "$PORT_32B" "32b_scorer" \
        "Qwen/Qwen3-VL-32B-Thinking" "0.90"
    SERVER_32B_PID=$!

    # Start 2B drafter on node 2
    start_server_on_node "$NODE2" "$PORT_2B" "2b_drafter" \
        "Qwen/Qwen3-VL-2B-Thinking" "0.50"
    SERVER_2B_PID=$!

    # Wait for both
    wait_for_server "$VLLM_32B_URL" "32B scorer" \
        "$RESULTS_DIR/vllm_server_32b_scorer.log" "$SERVER_32B_PID"
    wait_for_server "$VLLM_2B_URL"  "2B drafter" \
        "$RESULTS_DIR/vllm_server_2b_drafter.log" "$SERVER_2B_PID"

    python3 run_specReason.py
    stop_servers
fi
